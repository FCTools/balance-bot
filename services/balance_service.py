...
class BalanceService(metaclass=Singleton):
    ...
    def propeller_authorize(self):
        ...
        time.sleep(5)
        print(136, session.cookies)

        login_advertiser_page = requests_manager.get(
            session,
            "https://propellerads.com/login-advertiser/",
            headers={"User-Agent": self._user_agent,
                     "Referer": "https://propellerads.com/",
                     "Host": "partners.propellerads.com"},
            cookies = session.cookies
        )
        # print(login_advertiser_page.text)

        time.sleep(5)
        print(149, session.cookies)
        ...
        auth_response = requests_manager.post(
            ...
            headers={"User-Agent": self._user_agent,
                     "Host": "partners.propellerads.com",
                     "Referer": "https://partners.propellerads.com/",
                     "Origin": "https://partners.propellerads.com"},
            cookies=session.cookies,
        )

        time.sleep(5)
        print(170, session.cookies)

        logout_request = requests_manager.get(
            session,
            "https://partners.propellerads.com/v1.0/user/logout",
            headers={"User-Agent": self._user_agent},
            cookies=session.cookies
        )

        print(logout_request.json())
        print(179, session.cookies)
        time.sleep(5)

        solver = recaptchaV2Proxyless()
        solver.set_verbose(1)
        solver.set_key(self._captcha_api_key)
        solver.set_website_url("https://partners.propellerads.com/#/app/auth")
        solver.set_website_key("6Lf5WBwUAAAAAIgUvOTBsSG4EvBAUS7fXyqCI-yB")

        g_recaptcha_response = solver.solve_and_return_solution()
        print(g_recaptcha_response)

        auth_response_with_captcha = requests_manager.post(
            session,
            "https://partners.propellerads.com/v1.0/login_check",
            params={
                "captchaResponse": g_recaptcha_response,
                "fingerprint": self._networks["PropellerAds"]["fingerprint"],
                "isChina": False,
                "password": self._networks["PropellerAds"]["password"],
                "type": "ROLE_ADVERTISER",
        ...
        if not isinstance(auth_response_with_captcha, requests.Response):
            self._logger.error(f"Error occurred while trying to authorize on propeller: {auth_response_with_captcha}")
            return False

        if auth_response_with_captcha.status_code != 200:
            print(auth_response_with_captcha.json())
            self._logger.error(f"Can't authorize on propeller, request status code: {auth_response_with_captcha.status_code}.")
            return False

        try:
            request_json_data = auth_response_with_captcha.json()
        except json.JSONDecodeError as decode_error:
            self._logger.error(
        ...
        try:
            ...
            print(request_json_data["result"]["accessToken"])
        except KeyError:
            self._logger.error(
