...
from services.helpers.captcha_solver import CaptchaSolver
...
class KadamClient(TrafficSourceClient):
    def __init__(self, telegram_access_token):
        ...
        super().__init__(
            ...
            network_fullname="Kadam",
            network_alias="kadam",
            interface="web",
            login=os.getenv("KADAM_EMAIL"),
            password=os.getenv("KADAM_PASSWORD"))
    ...
    def _authorize(self):
        ...
        auth_page = requests_manager.get(
            session,
            "https://my.kadam.net/en/login",
            headers={"User-Agent": self._user_agent,
                     "Referer": "https://www.google.com/"})

        if not isinstance(auth_page, requests.Response):
            self._logger.error(f"Error occurred while trying to get login page for kadam: {auth_page}")
            return False

        if auth_page.status_code != 200:
            self._logger.error(f"Get kadam auth-page with non-success status code: {auth_page.status_code}."
        ...
        try:
            data_sitekey = str(soup.select("#login_form_recaptcha")[0]).split('"')[3]
        except IndexError:
            self._logger.error("Can't get data-sitekey (captcha key) from kadam auth page.")
            return False

        g_recaptcha_response = self._captcha_solver.solve(data_sitekey, "https://my.kadam.net/en/login")
